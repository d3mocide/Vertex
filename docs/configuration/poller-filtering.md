...
| Weather (`poller/pollers/weather.py`) | METAR/TAF/PIREP use BBOX query; AirNow AQI uses centerpoint + fixed 50 distance parameter. NWWS and PWS use configured office/station IDs. | `regions` or `BBOX_*`, `REGION_LAT/LON`, `NWS_OFFICE`, `WUNDERGROUND_*` |
...
| GDACS (`poller/pollers/gdacs.py`) | GeoRSS parsed globally; events are distance-gated from region center, with the threshold scaled by alert severity level. | `REGION_LAT/LON` |
...
5. For GDACS: low-severity events have tighter distance gates; only elevated-severity alerts appear at long range.
