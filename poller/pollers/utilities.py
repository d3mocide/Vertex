    ...
    async def poll(self):
        try:
            ...
            async with httpx.AsyncClient(
                ...
                pge_data = utility_stats.get(
                    "PORTLAND GENERAL ELECTRIC CO", {"affected": 0}
                )
                ...
                await set_feed(
                    "utility:oregon",
                    {
                        ...
                        "status": "Operational"
                        if state_total_affected < 1000
                        else "Regional Outages",
                ...
                await set_feed(
                    "utility:pge",
                    {
                        "provider": "PGE",
                        "status": "Operational"
                        if pge_data["affected"] < 100
                        else "Outages Detected",
                ...
                if self._consecutive_failures > 0:
                    logger.info(
                        "[utilities] Oregon ODIN feed recovered after %d failures",
