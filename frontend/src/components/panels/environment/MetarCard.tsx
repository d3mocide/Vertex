  ...
  return (
    <div className="hud-panel p-4 bg-onyx-deep/40">
      <div className="label-caps mb-3 flex items-center gap-2">
        <span className="ms text-[14px] leading-none text-violet-400" aria-hidden="true">bar_chart</span>
        METAR / TAF
        <span className="ml-auto font-mono text-[9px] lg:text-[11px] text-gray-500">{metars.length} station{metars.length !== 1 ? 's' : ''}</span>
      </div>
            ...
            <div key={i} className="border border-white/5 bg-white/[0.02] p-2 hover:bg-white/[0.04] transition-colors flex flex-col gap-1.5">
              <div className="flex items-center justify-between gap-2 border-b border-white/5 pb-1">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[10px] lg:text-[12px] font-black text-on-surface tracking-tighter">{m.station ?? '????'}</span>
                  <span className={`font-mono text-[8px] lg:text-[11px] font-bold px-1.5 py-0.5 rounded-sm bg-white/5 ${catCls}`}>
                    {m.flight_category ?? '--'}
                  </span>
                </div>
                <div className="font-mono text-[9px] lg:text-[11px] font-bold text-amber-gold">
                  {windDir(m.wind_dir)} {m.wind_kt != null ? `${m.wind_kt}kt` : '--'}
                  {m.gust_kt != null ? ` G${m.gust_kt}` : ''}
                </div>
              </div>
              
              <div className="flex items-center justify-between gap-4 text-[9px] lg:text-[11px] font-mono leading-none">
                <div className="flex gap-3">
                  <div className="flex gap-1">
                    <span className="text-on-surface-variant/40 uppercase text-[7px] lg:text-[11px]">T/D</span>
                    <span className="text-on-surface font-bold">
                      {m.temp_c != null ? `${Math.round(m.temp_c)}` : '--'}/{m.dewpoint_c != null ? `${Math.round(m.dewpoint_c)}` : '--'}°
                    </span>
                  </div>
                  <div className="flex gap-1">
                    <span className="text-on-surface-variant/40 uppercase text-[7px] lg:text-[11px]">VIS</span>
                    <span className="text-on-surface">{m.visibility_sm != null ? `${m.visibility_sm}` : '--'}</span>
                  </div>
                  <div className="flex gap-1">
                    <span className="text-on-surface-variant/40 uppercase text-[7px] lg:text-[11px]">ALT</span>
                ...
                {taf && (
                  <button
                    className={`text-[7px] lg:text-[11px] font-mono px-2 py-0.5 border transition-colors uppercase tracking-widest ${showTaf === String(i) ? 'bg-violet-400 text-black border-violet-400' : 'text-violet-400/60 border-violet-400/20 hover:bg-violet-400/10'}`}
              ...
              {showTaf === String(i) && taf && (
                <div className="p-1.5 bg-black/40 border border-violet-400/20 animate-in fade-in slide-in-from-top-1">
                  <p className="text-[8px] lg:text-[11px] font-mono text-violet-200/60 leading-tight italic break-words">
                    {taf.raw}
                  </p>
                </div>
              )}
