    ...
    <div className={`hud-panel p-4 bg-onyx-deep/60 relative overflow-hidden transition-all duration-500 ${isSevere ? 'border-l-2 border-l-red-600' : 'border-l-2 border-l-amber-gold'}`}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex flex-col">
          <span className={`text-[10px] lg:text-[11px] font-black uppercase tracking-widest ${isSevere ? 'text-red-500' : 'text-amber-gold'}`}>
            {alert.event}
          </span>
      ...
      <div className="flex items-center gap-3 mt-4">
        <div className="flex flex-col">
          <span className="text-[8px] lg:text-[11px] font-mono text-on-surface-variant uppercase tracking-widest">Expires</span>
          <span className="text-[10px] lg:text-[11px] font-mono text-on-surface uppercase font-bold">{new Date(alert.expires).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
      </div>
