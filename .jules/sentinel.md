...
## 2025-06-03 - [SSRF in Mesh Messages Endpoint]
**Vulnerability:** The `/mesh/messages` POST endpoint in `backend/routers/mesh.py` retrieved the target URL for the active MeshCore pyMC-Repeater from the `poller_sources` table and made an outbound HTTP POST request to it using `httpx.AsyncClient` without any URL validation.
**Learning:** Any endpoint that initiates outbound requests based on dynamically retrieved configuration (even if the source is the database rather than direct user input) is vulnerable to SSRF. An attacker with privileges to modify the `poller_sources` table could set the URL to an internal service (e.g., `http://127.0.0.1:8080`), tricking the backend into sending internal requests.
**Prevention:** Always validate dynamically retrieved URLs using the centralized `validate_safe_url` function from `security.py` to block non-public or loopback addresses before establishing outbound connections.
